-- ============================================================================
-- Discovery Assistant — Flujo de creación de websites (Vectis)
-- Ejecutar DESPUÉS de schema.sql. Rellena el flujo public_key = 'da_vectis'.
--
-- Formato de tarjetas: las preguntas de opción usan RADIO (una sola) o
-- CHECKBOX (varias). Cualquier opción "Otro"/"Otra" muestra automáticamente
-- un campo de texto para escribir una opción libre (lo maneja el widget).
--
-- OJO: esto BORRA y reemplaza las preguntas actuales de 'da_vectis'.
-- ============================================================================

-- Asegura que el flujo exista y esté activo
insert into public.flows (public_key, name, language, status, accent_color, notification_email)
values ('da_vectis', 'Vectis', 'es', 'ACTIVE', '#E7AB2E', 'contact@wearevectis.com')
...
-- Reemplaza las preguntas del flujo
delete from public.flow_questions
where flow_id = (select id from public.flows where public_key = 'da_vectis');

with f as (
  select id from public.flows where public_key = 'da_vectis'
)
